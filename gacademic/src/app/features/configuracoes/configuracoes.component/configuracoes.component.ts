...
export class ConfiguracoesComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    max_disciplinas_reprovadas: ['0'],
    limite_faltas_percentagem: [''],
    nota_maxima: [10, [Validators.required, Validators.min(0.01)]],
  ...
  ngOnInit() {
    ...
    this.store.select(selectConfiguracao).subscribe(config => {
      this.form.patchValue({
        ...
        max_disciplinas_reprovadas: String(config.max_disciplinas_reprovadas ?? 0),
        limite_faltas_percentagem: config.limite_faltas_percentagem != null ? String(config.limite_faltas_percentagem) : '',
  ...
  onGuardar() {
    ...
    this.store.dispatch(ConfiguracoesActions.atualizarConfiguracao({
      dados: {
        ...
        max_disciplinas_reprovadas: Number(v.max_disciplinas_reprovadas || 0),
        limite_faltas_percentagem: v.limite_faltas_percentagem ? Number(v.limite_faltas_percentagem) : null,
        nota_maxima: Number(v.nota_maxima),
