...
seguinte (ex.: setembro de 2026 a junho de 2027); ano_letivo_atual é
sempre a junção "YYYY/YYYY" dos dois anos (ex.: "2026/2027"), nunca só o
ano de início. Ver app/schemas/configuracoes.py::ConfiguracaoTenantUpdate
para a validação de intervalo/formato e app/database/models.py::Tenant
para os campos. Módulo novo, sem nenhum teste antes desta sessão (não
existia campo nenhum de ano letivo em Configurações)."""
...
async def test_guardar_ano_letivo_completo_e_devolvido_no_get(client):
    ...
    resp = await client.put("/api/v1/configuracoes", headers=headers, json=_payload_base(
        data_inicio_ano_letivo="2026-09-01", data_fim_ano_letivo="2027-06-30", ano_letivo_atual="2026/2027",
    ))
    assert resp.status_code == 200, resp.text
    corpo = resp.json()
    assert corpo["data_inicio_ano_letivo"] == "2026-09-01"
    assert corpo["data_fim_ano_letivo"] == "2027-06-30"
    assert corpo["ano_letivo_atual"] == "2026/2027"
...
async def test_ano_letivo_atual_fora_do_formato_yyyy_yyyy_e_rejeitado(client):
    escola = await criar_escola_e_gestor(client, "ano-letivo-formato")
    headers = auth_headers(escola["token"])

    for valor_invalido in ["2026", "2026-2027", "26/27", "2026/27"]:
        resp = await client.put("/api/v1/configuracoes", headers=headers, json=_payload_base(
            ano_letivo_atual=valor_invalido,
        ))
        assert resp.status_code == 422, f"{valor_invalido!r} devia ter sido rejeitado: {resp.text}"
