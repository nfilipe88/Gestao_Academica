...
# funil (ver _garantir_funil_seed) — nomes tirados literalmente do
...
# HELPERS
# ==========================================
async def _garantir_funil_seed(db: AsyncSession, tenant_id) -> list[FunilEtapa]:
...
async def criar_lead_publico(
    ...
    etapas = await _garantir_funil_seed(db, tenant_id)
...
async def listar_funil(
    ...
    etapas = await _garantir_funil_seed(db, utilizador["tenant_id"])
...
async def criar_etapa(
    ...
    await _garantir_funil_seed(db, tenant_id)  # garante que já há uma base antes de adicionar mais
...
async def listar_leads(
    ...
    leads = (await db.execute(
        select(LeadCandidato).where(LeadCandidato.tenant_id == utilizador["tenant_id"])
...
async def criar_lead_manual(
    ...
    return {"lead": novo_lead, "oportunidade_id": nova_oportunidade.id}
...
async def atualizar_lead(
    ...
    lead = (await db.execute(
        select(LeadCandidato).where(LeadCandidato.id == lead_id, LeadCandidato.tenant_id == utilizador["tenant_id"])
    )).scalars().first()
    if not lead:
...
async def mover_oportunidade(
    ...
    nova_etapa = (await db.execute(
        select(FunilEtapa).where(FunilEtapa.id == dados.nova_etapa_id, FunilEtapa.tenant_id == tenant_id)
    )).scalars().first()
    if not nova_etapa:
